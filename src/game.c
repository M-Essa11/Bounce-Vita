...
#define GAME_VIEW_HEIGHT 181
#else
#define GAME_VIEW_WIDTH SCREEN_WIDTH
#define GAME_VIEW_HEIGHT SCREEN_HEIGHT
#endif
...
static inline bool is_level_small(void) {
    int gameAreaHeight = GAME_VIEW_HEIGHT - HUD_HEIGHT;
...
static void game_calculate_camera(int* outCameraX, int* outCameraY) {
    ...
    int gameAreaHeight = GAME_VIEW_HEIGHT - HUD_HEIGHT;
    int cameraX = player->xPos - GAME_VIEW_WIDTH / 2;
    ...
    int maxCameraX = g_level.width * TILE_SIZE - GAME_VIEW_WIDTH;
...
static bool game_exit_is_visible(int cameraX, int cameraY) {
    ...
    const int gameAreaHeight = GAME_VIEW_HEIGHT - HUD_HEIGHT;

    return exitX < cameraX + GAME_VIEW_WIDTH &&
...
void game_init(void) {
    ...
    g_game.selected_level = save->has_continue ? save->current_level : 1;
    
    // Инициализация saved game state
    g_game.saved_game_state = save->has_continue ? SAVED_GAME_IN_PROGRESS : SAVED_GAME_NONE;
    ...
    g_game.numRings = 0;
    g_game.score = save->has_continue ? save->current_score : 0;
    g_game.numLives = save->has_continue ? save->current_lives : 3;
...
static void update_game(void) {
    ...
    Player* player = &g_game.player;
    const bool move_left = input_held(PSP_CTRL_LEFT) || input_held(PSP_CTRL_LTRIGGER);
    const bool move_right = input_held(PSP_CTRL_RIGHT) || input_held(PSP_CTRL_RTRIGGER);
    ...
    if (input_consume_pressed(PSP_CTRL_LEFT) || input_consume_pressed(PSP_CTRL_LTRIGGER) || move_left) {
        set_direction(player, MOVE_LEFT);
    }
    if ((input_consume_released(PSP_CTRL_LEFT) || input_consume_released(PSP_CTRL_LTRIGGER)) && !move_left) {
        release_direction(player, MOVE_LEFT);
    }

    // Движение ВПРАВО — учитываем и событие press/release, и удержание
    if (input_consume_pressed(PSP_CTRL_RIGHT) || input_consume_pressed(PSP_CTRL_RTRIGGER) || move_right) {
        set_direction(player, MOVE_RIGHT);
    }
    if ((input_consume_released(PSP_CTRL_RIGHT) || input_consume_released(PSP_CTRL_RTRIGGER)) && !move_right) {
    ...
    // Square + Triangle toggles invincibility.
    if ((input_pressed(PSP_CTRL_SQUARE) && input_held(PSP_CTRL_TRIANGLE)) ||
        (input_pressed(PSP_CTRL_TRIANGLE) && input_held(PSP_CTRL_SQUARE))) {
...
static void render_game(void) {
    ...
    level_render_visible_area(cameraX, cameraY, GAME_VIEW_WIDTH, gameAreaHeight);
    ...
    // Readable Vita HUD
    const int hudStartY = GAME_VIEW_HEIGHT - HUD_HEIGHT;
    int separator_y = hudStartY;
    int hud_blue_y = separator_y + 1;

    // Белый разделитель (1 пиксель)
    graphics_draw_rect(0, separator_y, GAME_VIEW_WIDTH, 1, COLOR_WHITE_ABGR);

    graphics_draw_rect(0, hud_blue_y, GAME_VIEW_WIDTH, HUD_HEIGHT - 1, 0xE8A04008);
    ...
    int score_width = graphics_measure_text(score_buffer, 12);
    int score_x = (GAME_VIEW_WIDTH - score_width) / 2;
    int score_y = hudStartY + 6;
    graphics_draw_text(score_x, score_y, score_buffer, 0xFF40FFFF, 12);
    if (tileset) {
        sprite_rect_t ring_icon = png_create_sprite_rect(tileset, TILE_SIZE, 4 * TILE_SIZE,
                                                         TILE_SIZE, TILE_SIZE);
        sprite_rect_t life_ball = png_create_sprite_rect(tileset, 2 * TILE_SIZE, 0, TILE_SIZE, TILE_SIZE);
        int remaining_rings = g_level.totalRings - g_game.numRings;
        int displayed_lives = g_game.numLives > 5 ? 5 : g_game.numLives;
        int displayed_rings = remaining_rings > 12 ? 12 : remaining_rings;
        for (int index = 0; index < displayed_rings; ++index) {
            png_draw_sprite(tileset, &ring_icon, 5 + index * 10, hudStartY + 5, 9, 12);
        }
        for (int index = 0; index < displayed_lives; ++index) {
            int x = GAME_VIEW_WIDTH - 8 - (displayed_lives - index) * 13;
            png_draw_sprite(tileset, &life_ball, x, hudStartY + 5, 10, 10);
        }
    }
