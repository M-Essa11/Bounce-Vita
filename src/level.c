...
static inline int get_max_sprite_index(void) {
    ...
    int source_scale = s_tileset->source_scale > 0 ? s_tileset->source_scale : 1;
    int tiles_per_col = s_tileset->actual_height / (TILE_SIZE * source_scale);
...
static void level_load_tileset_once(void) {
    if (s_tileset) return;
#ifdef BOUNCE_VITA
    s_tileset = png_load_texture_vram(TILESET_HD_PATH);
    if (s_tileset) {
        if (s_tileset->actual_width == 192 && s_tileset->actual_height == 288) {
            s_tileset->source_scale = 4;
        } else {
    ...
    if (s_tileset && s_tileset->width > 0) {
        int source_scale = s_tileset->source_scale > 0 ? s_tileset->source_scale : 1;
        s_tiles_per_row = s_tileset->actual_width / (TILE_SIZE * source_scale);
    } else {
        s_tiles_per_row = 0;
    }
}
