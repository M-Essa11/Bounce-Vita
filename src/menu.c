...
static void menu_render(void) {
...
#ifdef BOUNCE_VITA
    graphics_draw_rect(70, 35, 340, 210, 0xE8402818);
    graphics_draw_rect(70, 35, 340, 3, COLOR_SELECTION_BG);
#endif
    ...
    // Заголовок (по центру)
    {
        ...
        int x = center_x - (w / 2);
        graphics_draw_text(x, title_y, title, 0xFF40A0FF, font_height);
#ifdef BOUNCE_VITA
        {
            texture_t *tileset = level_get_tileset();
            if (tileset) {
                sprite_rect_t ball = png_create_sprite_rect(tileset, 2 * TILE_SIZE, 0,
                                                             TILE_SIZE, TILE_SIZE);
                png_draw_sprite(tileset, &ball, x - 34, title_y - 6, 28, 28);
            }
        }
#endif
    }
    ...
    for (int i = 0; i < MENU_ITEMS_COUNT; ++i) {
        ...
        if (is_continue_unavailable) {
            ...
            color = selected ? COLOR_TEXT_SELECTED : COLOR_WHITE_ABGR;
        }
