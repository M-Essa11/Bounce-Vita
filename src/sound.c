...
static int vita_audio_thread(SceSize args, void *argp) {
    ...
    psp_sample_t *samples = memalign(64, sample_count * sizeof(*samples));
    (void)args;
    (void)argp;
    if (!samples) return -1;

    while (g_audio_running) {
        ott_audio_callback(samples, sample_count, NULL);
        {
            int output_result = sceAudioOutOutput(g_audio_port, samples);
            if (output_result < 0) {
                g_vita_audio_status = output_result;
                break;
            }
        }
    }
...
#ifdef BOUNCE_VITA
    g_audio_port = sceAudioOutOpenPort(SCE_AUDIO_OUT_PORT_TYPE_BGM, 1024, PSP_SR,
        SCE_AUDIO_OUT_MODE_STEREO);
    if (g_audio_port < 0) return 10;
#else
...
#ifdef BOUNCE_VITA
    {
        int volumes[2] = {g_audio_volume, g_audio_volume};
        sceAudioOutSetVolume(g_audio_port,
            SCE_AUDIO_VOLUME_FLAG_L_CH | SCE_AUDIO_VOLUME_FLAG_R_CH, volumes);
    }
    g_audio_running = 1;
    g_audio_thread = sceKernelCreateThread("bounce_audio", vita_audio_thread,
        0x10000100, 0x10000, 0, 0, NULL);
    if (g_audio_thread < 0 || sceKernelStartThread(g_audio_thread, 0, NULL) < 0) {
        g_audio_running = 0;
        if (g_audio_thread >= 0) sceKernelDeleteThread(g_audio_thread);
        g_audio_thread = -1;
